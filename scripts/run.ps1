...
switch ($command) {
    "start" {
        try {
            mongo up --detach
            go run ${ProjectRoot}/cmd/storage-equipment-api-service
        } finally {
            mongo down
        }
    }
    ...
    "docker" {
         docker build -t filipskrabak/storage-equipment-webapi:local-build -f ${ProjectRoot}/build/docker/Dockerfile .
    }
    "openapi" {
